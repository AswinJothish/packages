...
const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose }) => {
  ...
  const [error, setError] = useState<'not_authenticated' | 'fetch_error' | null>(null);
  ...
  const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>, itemId: string, maxStock: number) => {
  ...
  const handleRemoveCartItem = async (Id: string) => {
    try {
      ...
      const response = await fetch(`${baseUrl}/order/deleteCartItem?userId=${userId}&cartItemId=${Id}`, {
        method: 'DELETE'
      });
  ...
  const getPriceByRoleAndQuantity = (item: CartItem): number => {
    ...
    if (offers && offers.length > 0) {
      const sortedOffers = [...offers].sort((a, b) => a.from - b.from);
  ...
  const calculateTotal = (): number => {
    ...
    return cartItems.reduce((total, item) => 
      total + (getPriceByRoleAndQuantity(item) * item.quantity), 0
    );
  };
        ...
        {cartItems.map((item) => (
          <div key={item._id} className="flex justify-center items-center gap-4 border-b pb-2 overflow-x-hidden">
            ...
            <div className="flex w-full justify-between">
              <div>
                <h3 className="overflow-hidden  font-medium text-base text-ellipsis capitalize whitespace-nowrap">
                ...
                <p className="text-sm">
                  ...
                  <span className="line-through font-thin text-gray-500">       
                   <span className=" text-sm  mt-0.5 font-sans font-medium">
                                                        &#8377;
                                                    </span>{item.productId.strikePrice}</span>
                </p>
                <p className="text-gray-800 font-medium">        <span className="md:text-base mr-0.5 text-xs  mt-0.5 font-sans font-medium">
                                                        &#8377;
                                                    </span>{getPriceByRoleAndQuantity(item)}</p>
              </div>
              <div className="flex justify-between flex-col items-center">
                <button onClick={() => openDialog(item._id)} className="text-neutral-400 text-xl">
                  <FaRegCircleXmark />
                </button>
                <div>
                  <input 
  ...
  return (
    <>
      ...
      <div
        className={`fixed right-0 top-14  md:w-96 w-5/6 bg-white shadow-xl transform transition-transform duration-300 ease-in-out z-50 ${
        ...
        {cartItems.length > 0 && !error && !loading && (
          <div className="absolute bottom-0 left-0 right-0 p-4 border-t bg-white">
            ...
            <div className="flex justify-between py-1">
              ...
              <span className="md:text-base  text-xs  mt-0.5 font-sans">
                                                        &#8377;
                                                    </span>
                {calculateTotal()}</span>
            </div>
            <div className="flex justify-between py-1">
              ...
              <span className="whitespace-nowrap">
                <span className="text-green-600 text-sm md:text-base font-sans">Free Delivery</span>
                <span className="line-through font-sans text-gray-500 text-sm font-medium md:text-base whitespace-nowrap">         
                <span className="md:text-base  text-xs  mt-0.5 font-sans ">
                                                        &#8377;
                                                    </span>50</span>
              </span>
        ...
        <div className="absolute w-full h-[100vh] inset-0 bg-black/60 flex justify-center items-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full text-center">
            ...
            <div className="flex justify-around">
              <button
                onClick={() => selectedItemId && handleRemoveCartItem(selectedItemId)}
