...
const OrderSection: React.FC = () => {
    ...
    const cancelOrder = async (orderId: string) => {
        try {
          const response = await fetch(`${baseUrl}/order/cancel-order?id=${orderId}`, {
            method: 'PUT',
          });
          ...
          if (result.ok) {
            ...
            toast.error(result.message || 'Something went wrong while cancelling the order');
          }
        } catch (error) {
    ...
    return (
        <div className="md:p-6">
            <div className='flex justify-between items-start mb-8'>
                ...
                <img src={filter} alt="filter image" className='w-8 h-8' onClick={() => { setDropdown(true) }} />
            </div>
            {dropdown &&
                        ...
                        orderData.map((order) => (
                            <div key={order._id} className="border border-gray-300  h-fit rounded-lg shadow-md bg-white">
                                <div className="flex justify-between items-center border-b-2 py-3 px-3">
                                    <div className={`md:px-4 capitalize md:py-2 px-2 py-1 rounded-full w-fit ${
                        ...
                        <>
                            ...
                            <div ref={dialogRef} className={`fixed right-0 top-0 h-full md:w-1/3 w-full bg-white shadow-xl transform transition-all  duration-500  ease-in-out z-50 ${isDialogOpen
                                ...
                                <div className="flex items-center overflow-x-hidden justify-between rounded-t-xl py-4 border-b-2 bg-gray-100">
                                    <h2 className="text-lg flex gap-2 items-center font-semibold px-4"> <span className='cursor-pointer' onClick={closeDialog}><IoMdArrowBack /></span>Order Details</h2>
                                    {/* <button onClick={closeDialog} className="p-1 hover:bg-gray-200 rounded-full">
                                        <X className="h-5 w-5" />
                                    </button> */}
                                 <button
  onClick={() => { setIsCancelDialogOpen(true); }}
  className={`mr-5 rounded-full p-1 px-3 text-white ${selectedOrderDetails.status !== 'pending' && selectedOrderDetails.status !== 'assigned' ? 'bg-red-500/50 cursor-not-allowed' : 'bg-red-600 cursor-pointer'}`}
  disabled={selectedOrderDetails.status !== 'pending' && selectedOrderDetails.status !== 'assigned'}
>
  Cancel Order
</button>
...
{isCancelDialogOpen && (
        ...
        >
          ...
          >
            <h3 className="text-lg font-semibold mb-4">Do you want to cancel this order?</h3>
            <div className="flex justify-center gap-4">
              <button
                className="bg-red-600 text-white px-4 py-2 rounded-full"
                onClick={() => cancelOrder(selectedOrderDetails._id)}
              >
                Confirm
              </button>
              <button
                ...
                Cancel
                                ...
                                <div className=" h-[calc(100vh-8rem)] overflow-y-scroll overflow-x-hidden bg-gray-100 flex flex-col">
                                    ...
                                    <div className='flex px-2 mx-1 mt-2 py-2 justify-between  items-center border bg-white rounded-xl  '>
                                        ...
                                        <p
  className={`px-3 py-2 rounded-full font-semibold text-sm text-white capitalize ${
    selectedOrderDetails.status === 'completed'
      ...
      ? 'bg-blue-500'
      : selectedOrderDetails.status === 'assigned'
      ? 'bg-teal-500'
      : selectedOrderDetails.status === 'picked'
      ? 'bg-pink-700'
      : selectedOrderDetails.status === 'outfordelivery'
      ? 'bg-orange-500'
...
>
  {selectedOrderDetails.status === 'pending' ? 'InProgress': selectedOrderDetails.status === 'outfordelivery' 
    ? 'Out For Delivery'  : selectedOrderDetails.status}
</p>

                                    </div>

                                    {
                                        selectedOrderDetails.status === 'completed' || selectedOrderDetails.status == 'cancelled'||selectedOrderDetails.status == 'rejected' ? <></> : <><div className='mt-2 mx-1 bg-white px-2 py-2 rounded-xl border'>
                                    ...
                                    <div className='w-[100%] px-5 mt-5'>
                                        <div className='flex flex-row overflow-x-auto w-[100%] gap-5 '>
                                            {selectedOrderDetails.paymentDetails.map((detail: { _id: Key | null | undefined; paidAmount: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | null | undefined; balanceAmount: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | null | undefined; paymentImage: any; transactionId: any; mode: string; }, index: number) => (
                                                <div className='w-[100%]'>
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 p-4 border-t bg-white rounded-xl">
                                        ...
                                        ) : selectedOrderDetails.status === "cancelled"||selectedOrderDetails.status === "rejected" ? (
                                            <button
                                                disabled
                                                className="w-full bg-red-600 font-bold text-white py-2 rounded-lg hover:bg-red-700"
                                            >
                                               {selectedOrderDetails.status==='cancelled'?' Order Cancelled':' Order Rejected'}
                                            </button>
                                        ) : (
                                            <button
