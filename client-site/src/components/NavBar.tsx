...
const[isHome,setIsHome]=useState(false);
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const data = await response.json();
      const simplifiedProducts = (data.products || []).map((product: { _id: any; productName: any; productImages: any[]; }) => ({
        _id: product._id,
  ...
  return (
    <div   className="z-40 ">
      <div ref={navBarRef} className='z-40 shadow-md  h-16 nav-bar fixed w-full bg-white  flex items-center  justify-between px-4 ' >
      <div className="w-1/4">
      ...
      <div className="flex gap-10 w-2/3 justify-end items-center">
            ...
            <div className="absolute w-full mt-1 bg-white border rounded-lg shadow-lg max-h-[80vh] overflow-y-auto z-50">
              {loading ? (
                ...
                <div className="px-4 py-2 text-neutral-500">
                  ...
                  {lastFiveProducts.map((product:any) => (
                    <div className='flex border-b p-2 items-center' key={product?._id}>
                      <div className='border flex justify-center items-center'>
                        <img src={`${ImgbaseUrl}${product?.image}`} alt="product image" className='w-12 h-12 p-1' />
                      </div>
        ...
        <div className='flex items-center gap-5'>
          <button  onClick={() => setIsCartOpen((prev) => !prev)} className=""><img src={shopingCart} alt="" className='w-6 h-6' /></button>
          <CartDrawer 
    isOpen={isCartOpen} 
  ...
  >
          ...
          <div ref={profileDropdownRef} className="relative inline-block">
            ...
            <div className='flex justify-center items-center flex-col gap-1'>
              <button
                ...
                className='py-2 text-left px-3 w-full hover:bg-neutral-200'
              >
                Profile
              </button>
              <button
      ...
      <div className='pt-[72px]  md:pt-0 w-full'>
      <div className={`w-full px-1  pb-1  ${isHome?'md:hidden block':'hidden'} relative`}>
          <div className="relative">
            <input
              type="text"
              placeholder="Search products..."
          ...
          {isSearchFocused && (
            <div className="absolute w-full mt-1  flex justify-center overflow-x-hidden  ">
              <div className='w-[90%] bg-white border rounded-lg shadow-lg max-h-[70vh] overflow-y-auto z-50'>
              {loading ? (
                <div className="px-4 py-2 text-neutral-500">Loading...</div>
              ) : filteredProducts.length > 0 ? (
                filteredProducts.map((product:any) => (
                  <div className='flex border-b p-2 items-center' key={product._id}>
                    ...
                    <div
                      className="w-full px-2 py-1 text-sm hover:bg-neutral-50 cursor-pointer"
                      onClick={() => handleProductSelect(product)}
                    >
                      {product.title}
                    </div>
                ...
                <div className="px-2 py-1 text-zinc-600">
                  ...
                  {lastFiveProducts.map((product) => (
                    <div className='flex border-b p-1 items-center' key={product._id}>
                      ...
                      <div
                        className="px-2 text-sm py-1 w-full hover:bg-neutral-50 cursor-pointer"
                        onClick={() => handleProductSelect(product)}
                      >
                        {product.title}
                      </div>
