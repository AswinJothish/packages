...
.post(
  ...
  async ({ body }) => {
    try {
      ...
      const generateEmployeeId = async (): Promise<string> => { // Add return type annotation here
        const randomId = `EMP-${Math.floor(1000 + Math.random() * 9000)}`;
        const existingAgent = await DeliveryAgent.findOne({ employeeId: randomId });
        return existingAgent ? generateEmployeeId() : randomId;
      };

      uniqueEmployeeId = await generateEmployeeId();

      // Create the DeliveryAgent
